...
std::vector<Point>     fiberPositions_babybcal(double r, double sx, double sz, double trx, double trz, double phi,
                                      bool shift_first = false, double stol = 1e-2);
std::vector<FiberGrid> gridPoints_babybcal(int div_n_phi, double div_dr, double x, double z, double phi);

// geometry helpers
void buildFibers_babybcal(Detector& desc, SensitiveDetector& sens, Volume& mother, int layer_nunber, xml_comp_t x_fiber,
                 const std::tuple<double, double, double, double>& dimensions);
void buildSupport_babybcal(Detector& desc, Volume& mother, xml_comp_t x_support,
                  const std::tuple<double, double, double, double>& dimensions);
...
static Ref_t create_detector(Detector& desc, xml_h e, SensitiveDetector sens)
{
  ...
  Transform3D  tr_global = Translation3D(0, 0, offset) * RotationZ(-hphi);  // Changed: hphi -> -hphi
  ...
  for (int i = 0; i < nsectors; i++, phi -= dphi) { // i is module number
  ...
  if (x_det.hasChild(_U(support))) {
    buildSupport_babybcal(desc, mod_vol, x_det.child(_U(support)), {inner_r, l_pos_z, x_dim.z(), hphi});
  }
...
void buildFibers_babybcal(Detector& desc, SensitiveDetector& sens, Volume& s_vol, int layer_number, xml_comp_t x_fiber,
                 const std::tuple<double, double, double, double>& dimensions)
{
...
void buildSupport_babybcal(Detector& desc, Volume& mod_vol, xml_comp_t x_support,
                  const std::tuple<double, double, double, double>& dimensions)
{
...
std::vector<Point> fiberPositions_babybcal(double r, double sx, double sz, double trx, double trz, double phi, bool shift,
                                  double stol)
{
  // r      - fiber radius
...
std::vector<FiberGrid> gridPoints_babybcal(int div_n_phi, double div_dr, double trd_x1, double height, double phi)
{
  /*
...
DECLARE_DETELEMENT(epic_EcalBabyBarrelScFi, create_detector)
