...
// geometry helpers
static void buildSupport_babybcal(Detector& desc, Volume& mother, xml_comp_t x_support,
...
static Ref_t create_detector(Detector& desc, xml_h e, SensitiveDetector sens) {
  ...
  double offsetX            = x_detector.attr<double>(_Unicode(offsetX));
  double offsetY            = x_detector.attr<double>(_Unicode(offsetY));
  double offsetZ            = x_detector.attr<double>(_Unicode(offsetZ));
  xml_comp_t x_dimensions   = x_detector.dimensions();
  int nsides                = x_dimensions.numsides();
  int nsectors              = x_dimensions.attr<int>(_Unicode(nsectors));
  ...
  //Transform3D detector_tr       = Translation3D(offsetX, offsetY, offsetZ) * RotationZ(-half_dphi-(M_PI*0.5));
  Transform3D detector_tr       = Translation3D(offsetX, offsetY, offsetZ) * RotationZ(-(dphi/2)-(M_PI*0.5));
  ...
  if (x_detector.hasChild(_Unicode(support))) {
    buildSupport_babybcal(desc, sector_volume, x_detector.child(_Unicode(support)),
  ...
  for (int i = 0; i < nsectors; i++, phi -= dphi) { // i is sector number
...
static void buildSupport_babybcal(Detector& desc, Volume& sector_volume, xml_comp_t x_support,
                         const std::tuple<double, double, double, double>& dimensions) {
...
DECLARE_DETELEMENT(epic_EcalBabyBarrelImaging, create_detector)
